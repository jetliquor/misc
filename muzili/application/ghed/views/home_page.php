 <div id="content"> 
	<div id="myCarousel" class="carousel slide" align="center">
		<ol class="carousel-indicators">
			<li data-target="#myCarousel" data-slide-to="0" class="active"></li>
			<li data-target="#myCarousel" data-slide-to="1"></li>
			<li data-target="#myCarousel" data-slide-to="2"></li>
		</ol>
		<!-- Carousel items -->
		<div class="carousel-inner">
			<div class="item active" >
				<img style="position: relative;" class="img-polaroid1"
					src="<?php echo get_img_path().'bootstrap-mdo-sfmoma-04-new.jpg'?>"
					 title="">
				<div class="carousel-caption">
					
				</div>
			</div>
			<div class="item">
				<img title="2" class="img-polaroid1"
					src="<?php echo get_img_path().'bootstrap-mdo-sfmoma-05.jpg'?>" />
			</div>
			<div class="item">
				<img title="3" class="img-polaroid1"
					src="<?php echo get_img_path().'bootstrap-mdo-sfmoma-06.jpg'?>" />
			</div>
		</div>
		<!-- Carousel nav -->
		<a class="carousel-control left" href="#myCarousel"
			data-slide="prev"><label class="carousel-label">&lsaquo;</label></a>
		<a class="carousel-control right" href="#myCarousel"
			data-slide="next"><label class="carousel-label">&rsaquo;</label></a>
	</div>
</div>
